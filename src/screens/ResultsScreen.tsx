...
export function ResultsScreen({ navigate }: { navigate: (screen: ScreenName) => void }) {
  ...
  if (!lastResult) return <View style={styles.container}><AppHeader eyebrow="RESULTS" title="No experiment yet." subtitle="Run a scenario to populate the decision record."/><Button title="Open use cases" onPress={() => navigate('useCases')} /></View>;
  ...
  return <View style={styles.container}><AppHeader eyebrow="EXPERIMENT COMPLETE" title="The result is a business decision." subtitle={`${lastResult.runId} · ${lastResult.durationMs} ms · ${lastResult.solver}`} badge={lastResult.status === 'fallback' ? 'LOCAL FALLBACK' : 'COMPLETE'} />
