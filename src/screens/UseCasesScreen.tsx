...
export function UseCasesScreen({ navigate }: { navigate: (screen: ScreenName) => void }) {
  ...
  return <Screen><View style={styles.container}><AppHeader eyebrow="PROBLEM CATALOG" title="Four paths to value." subtitle="Each path uses the same product loop: model → baseline → optimize → explain → experiment." />
...
const styles = StyleSheet.create({
  ...
  quote: { color: colors.cyan, fontSize: 20, fontWeight: '800', marginBottom: 4 }
});
